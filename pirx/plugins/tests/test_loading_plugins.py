...
def test_loader_loads_plugins_list_from_given_path(mocker):
    ...
    list_mock = mocker.patch(
        "pirx.plugins.loader.load_plugin_list_if_exists", return_value=None
    )
...
def test_loader_creates_plugin_instance_for_plugin_name(mocker):
    plugin_name = "plugin"
    mocker.patch(
        "pirx.plugins.loader.load_plugin_list_if_exists", return_value=[plugin_name]
    )
    plugin = mocker.patch("pirx.plugins.loader.Plugin")
...
def test_loader_is_not_adding_non_local_plugin_to_sys_path(mocker):
    ...
    mocker.patch("pirx.plugins.loader.Plugin")
...
def test_loader_adds_local_plugin_path_to_sys_path(mocker):
    ...
    path_mock = mocker.patch("sys.path", Mock(append=Mock()))
    plugin_mock = mocker.patch("pirx.plugins.loader.Plugin")
...
def test_loader_imports_plugin_module_if_it_exists(mocker):
    ...
    mocker.patch(
        "pirx.plugins.loader.load_plugin_list_if_exists", return_value=["plugin"]
    )
    plugin_mock = Mock(import_module_if_exists=Mock(return_value="imported"))
    mocker.patch("pirx.plugins.loader.Plugin", Mock(return_value=plugin_mock))
